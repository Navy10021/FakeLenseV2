...
def mock_inference_engine():
    ...
    mock_engine.predict_with_confidence.return_value = {
        ...
        "label": "Real News",
        "confidence": 0.95,
        "all_probabilities": {"fake": 0.02, "suspicious": 0.03, "real": 0.95},
    }
    return mock_engine
...
def test_predict_endpoint_without_optional_fields(mock_engine, client):
    ...
    mock_engine.predict_with_confidence.return_value = {
        ...
        "label": "Fake News",
        "confidence": 0.88,
        "all_probabilities": {"fake": 0.88, "suspicious": 0.10, "real": 0.02},
    }
...
def test_predict_endpoint_text_too_short(client):
    ...
    response = client.post("/predict", json={"text": "Short", "source": "Test"})
...
def test_batch_predict_too_many_articles(mock_engine, client):
    ...
    articles = [
        {"text": f"Article {i} with sufficient length.", "source": "Test"}
        for i in range(101)
    ]
...
def test_cors_headers(client):
    ...
    assert response.status_code in [200, 405]  # May vary by implementation
...
def test_confidence_scores_range(mock_engine, client):
    ...
    mock_engine.predict_with_confidence.return_value = {
        ...
        "confidence": 0.65,
        "all_probabilities": {"fake": 0.20, "suspicious": 0.65, "real": 0.15},
    }
...
def test_all_prediction_classes(mock_engine, client):
    ...
    for prediction, label, confidence in test_cases:
        mock_engine.predict_with_confidence.return_value = {
            ...
            "all_probabilities": {"fake": 0.33, "suspicious": 0.33, "real": 0.34},
        }
